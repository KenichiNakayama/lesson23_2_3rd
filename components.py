...
def display_select_mode():
    ...
    """
    # 問題3 アプリ見た目の修正: ラジオボタンをサイドバーに移動
    # サイドバーに回答モードを選択する用のラジオボタンを表示
    with st.sidebar:
        st.markdown("### 利用目的")
...
def display_initial_ai_message():
    """
    AIメッセージの初期表示とサイドバーに入力例を表示
    """
    # 問題3 アプリ見た目の修正: 入力例と機能説明をサイドバーに移動
    # サイドバーに入力例を表示
    with st.sidebar:
        st.markdown("### 使い方")
    ...
    # 問題3 アプリ見た目の修正: メインエリアを簡潔にして、チャットに集中できるように変更
    # メインエリアには簡潔なAIメッセージのみ表示
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        # 問題4 参照ページの表示: ページ番号が取得できた場合のみ、ページ番号を表示（ドキュメントによっては取得できない場合がある）
